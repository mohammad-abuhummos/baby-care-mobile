...
export default function CompleteSignUpBabyidScreen({navigation}) {
  const {user, setUserAuth, bracelet, setIsSignUp} = React.useContext(
    UserContext,
  );
  ...
  const handleInfo = (firstname, lastname, phone) => {
    ...
    if (User_info.isValid()) {
      ...
      database()
        ...
        .then(() => {
          navigation.navigate('EnterBabyid');
        });
    } else {
      setLoading(false);
